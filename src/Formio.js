define([
    ...
    'lodash/get',
    'lodash/cloneDeep'
], function (Promise, fetchPonyfill, EventEmitter, cookies, copy, providers, _get, _cloneDeep) {
    'use strict';
    const {fetch, Headers} = fetchPonyfill({ Promise: Promise });
    ...
    return class Formio {
        ...
        static request(url, method, data, header, opts) {
            ...
            const result = Formio.fetch(url, options).then(response => {
        ...
        static pluginWait(pluginFn, ...args) {
            return Promise.all(Formio.plugins.map(plugin => (plugin[pluginFn] || Formio.noop).call(plugin, ...args)));
        }
        static pluginGet(pluginFn, ...args) {
            const callPlugin = index => {
                ...
                return Promise.resolve((plugin[pluginFn] || Formio.noop).call(plugin, ...args)).then(result => {
                    if (!isNil(result)) {
                        return result;
                    }

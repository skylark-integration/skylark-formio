define([
    ...
    './Formio',
    './utils/utils'
], function (Promise, _, Webform, Base, Formio, a) {
    'use strict';
    return class Wizard extends Webform {
        ...
        getPages(args = {}) {
            ...
            const pages = this.pages.filter(all ? _.identity : (p, index) => this._seenPages.includes(index)).map((page, index) => this.createComponent(page, _.assign(pageOptions, { components: index === this.page ? components : null })));
        ...
        nextPage() {
            ...
            if (this.checkCurrentPageValidity(this.submission.data, true)) {
                this.checkData(this.submission.data, { noValidate: true });
        ...
        buildPages(form) {
            ...
            form.components.forEach(component => {
                if (component.type === 'panel') {
                    if (a.checkCondition(component, this.data, this.data, this.wizard, this)) {
        ...
        buildWizardHeader() {
            ...
            this.wizardHeader = this.ce('nav', { 'aria-label': 'navigation' });
            this.wizardHeaderList = this.ce('ul', { class: 'pagination' });
            ...
            this.pages.forEach((page, i) => {
                const clickableFlag = a.firstNonNil([
                ...
                const pageLabel = this.ce('span', { class: 'page-link' });
        ...
        onChange(flags, changed) {
            ...
            this.wizard.components.forEach(component => {
                ...
                if (a.hasCondition(component)) {
                    const hasPage = this.pages && this.pages[pageIndex] && this.pageId(this.pages[pageIndex]) === this.pageId(component);
                    const shouldShow = a.checkCondition(component, this.data, this.data, this.wizard, this);
        ...
        buildWizardNav(nextPage) {
            ...
            this.wizardNav = this.ce('ul', { class: 'list-inline' });
            ...
            [
                ...
                const buttonWrapper = this.ce('li', { class: 'list-inline-item' });
                const buttonProp = `${ button.name }Button`;
                const buttonElement = this[buttonProp] = this.ce('button', { class: `${ button.class } btn-wizard-nav-${ button.name }` });

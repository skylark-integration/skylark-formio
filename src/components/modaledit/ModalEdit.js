define([
    ...
    '../textarea/TextArea',
    '../../utils/utils.js'
], function (_, Formio, TextAreaComponent, a) {
    'use strict';
    ...
    return class ModalEditComponent extends TextAreaComponent {
        ...
        constructor(...args) {
            super(...args);
            const [get, toggle] = a.withSwitch(VIEW, EDIT);
            this.getMode = get;
            this.toggleMode = () => {
                toggle();
        ...
        buildEditMode({onCloseRequest, onCloseClick, onOverlayClick}) {
            ...
            const close = this.ce('button', {
                type: 'button',
                class: 'btn btn-primary btn-xs formio-modaledit-close'
            }, 'Close');
            const container = this.ce('div', { class: 'formio-modaledit-content' }, [
                close,
                inner
            ]);
            const dialog = this.ce('div', { class: 'formio-dialog formio-dialog-theme-default formio-modaledit-dialog' }, [
                overlay,
                container
            ]);
            const [dw, dh] = this.defaultEditorSize;
            const layout = _.get(this.component, 'editorLayout', this.defaultLayout);
            const widthPath = _.get(this.layoutOptions, [
                layout,
                'width'
            ]);
            const heightPath = _.get(this.layoutOptions, [
                layout,
                'height'
            ]);
        ...
        addCKE(element, settings, onChange) {
            ...
            return Formio.requireLibrary('ckeditor', 'InlineEditor', CKEDITOR, true).then(() => {

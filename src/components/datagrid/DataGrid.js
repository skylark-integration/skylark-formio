define([
    ...
    '../base/Base'
], function (_, NestedComponent, BaseComponent) {
    'use strict';
    return class DataGridComponent extends NestedComponent {
        ...
        hasRemoveButtons() {
            return !this.component.disableAddingRemovingRows && !this.shouldDisable && !this.options.builder && this.dataValue.length > _.get(this.component, 'validate.minLength', 0);
        }
        hasTopSubmit() {
        ...
        createHeader() {
            ...
            const thead = this.ce('thead', null, this.ce('tr', null, [
                this.allowReorder ? this.ce('th', { class: 'formio-drag-column-header' }) : null,
        ...
        buildRow(rowData, index, state) {
            ...
            if (this.allowReorder) {
                firstColumn = this.ce('td', { class: 'formio-drag-column' }, this.dragButton());
            }
            if (hasRmButton) {
                if (rmPlacement === 'col') {
                    lastColumn = this.ce('td', { class: 'formio-remove-column' }, this.removeButton(index));
                } else {
                    useCorner = true;
                }
            ...
            const rowElement = this.ce('tr', null, [
                firstColumn,
                components.map((cmp, colIndex) => {
                    const cell = this.buildComponent(cmp, colIndex, rowData, index, this.getComponentState(cmp, state));
            ...
            if (this.allowReorder) {
                rowElement.dragInfo = { index: index };
            }
            return rowElement;
        }
        destroyRows() {
        ...
        buildComponent(col, colIndex, row, rowIndex, state) {
            ...
            const comp = this.createComponent(_.assign({}, column, { row: options.row }), options, row, null, state);
        ...
        buildGroup({label}, index, groupRows) {
            ...
            const header = this.ce('tr', { class: `datagrid-group-header ${ hasToggle ? 'clickable' : '' }` }, cell);
            if (hasToggle) {

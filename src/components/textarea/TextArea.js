define([
    ...
    'lodash',
    '../../utils/utils'
], function (TextFieldComponent, Formio, _, a) {
    'use strict';
        ...
        createInput(container) {
            ...
            if (this.isPlain) {
                if (this.options.readOnly) {
                    this.input = this.ce('div', { class: 'well' });
            ...
            this.input = this.ce('div', { class: 'formio-wysiwyg-editor' });
            container.appendChild(this.input);
            this.addCounter(container);
            if (this.component.editor === 'ace') {
                this.editorReady = Formio.requireLibrary('ace', 'ace', 'https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.1/ace.js', true).then(() => {
            ...
            this.editorReady = this.addQuill(this.input, this.component.wysiwyg, () => {
            ...
            function imageHandler() {
                ...
                if (fileInput == null) {
                    ...
                    fileInput.addEventListener('change', () => {
                        ...
                        _this.root.formio.uploadFile(uploadStorage, files[0], a.uniqueName(files[0].name), uploadDir || '', null, uploadUrl, uploadOptions).then(result => {
                            return _this.root.formio.downloadFile(result);
                        }).then(result => {
                            this.quill.enable(true);
                            const Delta = Quill.import('delta');
                            this.quill.updateContents(new Delta().retain(range.index).delete(range.length).insert({ image: result.url }), Quill.sources.USER);
                            fileInput.value = '';
                        }).catch(error => {

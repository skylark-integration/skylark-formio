define([
    ...
    '../base/Base',
    'lodash'
], function (SignaturePad, BaseComponent, _) {
    'use strict';
    return class SignatureComponent extends BaseComponent {
        ...
        build() {
            ...
            this.refresh = this.ce('a', { class: 'btn btn-sm btn-default btn-light signature-pad-refresh' });
            ...
            if (_.get(this.component, 'validate.required', false)) {
                this.padBody.appendChild(this.ce('span', { class: 'form-control-feedback field-required-inline text-danger' }, this.getIcon('asterisk')));
            }
            this.signatureImage = this.ce('img', { style: 'width: 100%;display: none;' });
            this.padBody.appendChild(this.signatureImage);
            this.element.appendChild(this.padBody);
            if (this.component.footer) {
                this.signatureFooter = this.ce('div', { class: 'signature-pad-footer' });
            ...
            this.signaturePad.onEnd = () => this.setValue(this.signaturePad.toDataURL(), { noSign: true });

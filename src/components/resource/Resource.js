define([
    ...
    '../../Webform',
    '../../Formio'
], function (_, SelectComponent, Webform, Formio) {
    'use strict';
    return class ResourceComponent extends SelectComponent {
        ...
        addButton() {
            const addButton = this.ce('button', { class: 'btn btn-primary' });
        ...
        addInput(input, container) {
            if (this.component.addResource) {
                ...
                const template = '<tbody>' + '<tr>' + '<td id="select">' + '</td>' + '</tr>' + '<tr>' + '<td id="button" colspan="2">' + '</td>' + '</tr>' + '</tbody>';

define(['../nested/NestedComponent'], function (NestedComponent) {
    'use strict';
    return class PanelComponent extends NestedComponent {
        ...
        build(state) {
            ...
            this.panelBody = this.ce('div', { class: 'card-body panel-body' });
            if (this.component.title && !this.component.hideLabel) {
                const heading = this.ce('div', { class: `card-header bg-${ this.component.theme } panel-heading` });
                this.panelTitle = this.ce('h4', { class: 'mb-0 card-title panel-title' });

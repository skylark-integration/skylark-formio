define([
    ...
    '../base/Base',
    'lodash'
], function (NestedComponent, Base, _) {
    'use strict';
    return class TabsComponent extends NestedComponent {
            ...
            if (this.options.flatten) {
                ...
                this.component.components.forEach(tab => {
                    ...
                    const panel = this.ce('div', {
                        ...
                        this.ce('div', { class: 'card-header bg-default panel-heading' }, this.ce('h4', { class: 'mb-0 card-title panel-title' }, tab.label)),
                        body = this.ce('div', { class: 'card-body panel-body' })
                    ]);
                    tab.components.forEach(component => this.addComponent(component, body, this.data, null, null, this.getComponentState(component, state)));
        ...
        createElement() {
            this.tabsBar = this.ce('ul', { class: 'nav nav-tabs' });
            this.tabsContent = this.ce('div', { class: 'tab-content' });
            this.tabLinks = [];
            this.tabs = [];

define([
    ...
    'use strict';
    return class SurveyComponent extends BaseComponent {
        ...
        build() {
            if (this.viewOnly) {
                ...
                this.table = this.ce('table', { class: 'table table-striped table-bordered' });
                ...
                _.each(this.component.values, value => {
                    const th = this.ce('th', { style: 'text-align: center;' });
                ...
                _.each(this.component.questions, question => {
                    ...
                    tr.appendChild(td);
                    _.each(this.component.values, value => {
                        const td = this.ce('td', { style: 'text-align: center;' });
        ...
        getView(value) {
            ...
            const table = this.ce('table', { class: 'table table-striped table-bordered table-condensed' });
            const tbody = this.ce('tbody');
            _.each(value, (value, question) => {
                const row = this.ce('tr');
                const questionCell = this.ce('th');
                const valueCell = this.ce('td');
                const questionText = _.find(this.component.questions, [
                    'value',
                    question
                ]).label;
                const valueText = _.find(this.component.values, [
                    'value',
                    value
                ]).label;

define(['../base/Base'], function (BaseComponent) {
    'use strict';
    return class UnknownComponent extends BaseComponent {
        static schema() {
            return {
                type: 'custom',
                key: 'custom',
        ...
        build() {
            ...
            if (this.options.builder) {
                const builderElement = this.ce('div', { class: 'panel panel-default' }, [this.ce('div', { class: 'panel-body text-muted text-center' }, [document.createTextNode(`${ this.t('Custom Component') } (${ this.t(this.component.type) })`)])]);
